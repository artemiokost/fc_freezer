    ...
    loop {
        terminal.draw(|f| {
            ...
            let chunks = Layout::default()
                ...
                .margin(1)
                .constraints([Constraint::Length(3), Constraint::Min(10), Constraint::Length(7), Constraint::Length(3)].as_ref())
                .split(area);
            let s = state.lock().unwrap();

            let header = Paragraph::new(" 🔥 EA SPORTS FC 26 - KERNEL CHEATARMY EXTENSION v4.5 ")
                .style(Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD))
                .alignment(ratatui::layout::Alignment::Center)
                .block(Block::default().borders(Borders::ALL).border_style(Style::default().fg(Color::DarkGray)));

            // ИСПРАВЛЕНО: Индекс 0 для заголовка
            f.render_widget(header, chunks[0]);

            let mut active_str = format!(" Процесс игры:       {}\n\n MODS STATUS:\n", if s.game_pid == 0 { String::from("Ожидание запуска...") } else { format!("FC26.exe (PID: {})", s.game_pid) });
            ...
            let main_panel = Paragraph::new(active_str)
            ...
            // ИСПРАВЛЕНО: Индекс 1 для главной панели управления модами
            ...
            // ИСПРАВЛЕНО: Индекс 3 для подвала с подсказками
